...
function withDB(handler, { useTransaction = false } = {}) {
...
app.post('/api/users', withDB(async (req, res, connection, next) => {
...
app.post('/api/login', withDB(async (req, res, connection, next) => {
...
app.get('/api/ingredients', authenticateToken(false), withDB(async (req, res, connection, next) => {
...
app.post('/api/ingredients', authenticateToken(true), withDB(async (req, res, connection, next) => {
...
app.delete('/api/ingredients/:ingredient_id', authenticateToken(true), withDB(async (req, res, connection, next) => {
...
// Drink endpoints
app.get('/api/drinks', authenticateToken(false), withDB(async (req, res, connection, next) => {
...
app.get('/api/drinks/:drink_id', authenticateToken(false), withDB(async (req, res, connection, next) => {
    ...
    let query = `
        ...
        AND d.drink_id = ?
        AND d.created_user_id IN (1, ?)
    `
    let queryParams = [drink_id, user_id];
    ...
    for (const row of rows) {
        ...
        if (drink === undefined) {
            drink = {
                id: drink_id,
                create_time: JSDateToSwift(drink_time),
...
app.post('/api/drinks', authenticateToken(true), withDB(async (req, res, connection, next) => {
...
app.delete('/api/drinks/:drink_id', authenticateToken(false), withDB(async (req, res, connection, next) => {
    const { drink_id } = req.params;
    const user_id = req.user_id;
...
app.post('/api/sessions', authenticateToken(true), withDB(async (req, res, connection, next) => {
    const { start_time = new Date().toISOString() } = req.body;
    const user_id = req.user_id;
    const [[[{ new_session_id}]]] = await connection.query('CALL createSession(?, ?)', [user_id, JSDateToSQL(new Date(start_time))]);
    res.status(201).json({ new_session_id });
    
}));

app.post('/api/sessions/:session_id/sessiondrinks', authenticateToken(true), withDB(async (req, res, connection, next) => {
    ...
    const [[[{ new_pairing_id }]]] = await connection.query(
        ...
        [session_id, drink_id, quantity, JSDateToSQL(new Date(start_time)), end_time === null ? null : JSDateToSQL(new Date(end_time))]
    );
    res.status(201).send({ new_pairing_id });
}));

app.patch('/api/sessions/:session_id/sessiondrinks/:session_drink_id', authenticateToken(true), withDB(async (req, res, connection, next) => {
    const { session_id, session_drink_id } = req.params;
    const user_id = req.user_id;
    const { end_time = new Date().toISOString() } = req.body;
    ...
    await connection.query(
        'CALL finishSessionDrink(?, ?, ?)',
        [session_drink_id, user_id, JSDateToSQL(new Date(end_time))]
    );
    console.log("success")
    res.status(204).send();
}));

app.delete('/api/sessions/:session_id', authenticateToken(true), withDB(async (req, res, connection, next) => {
    const { session_id } = req.params;
    const user_id = req.user_id;
...
app.delete('/api/sessions/:session_id/sessiondrinks/:session_drink_id', authenticateToken(true), withDB(async (req, res, connection, next) => {
    const { session_id, session_drink_id } = req.params;
    const user_id = req.user_id;
    if (session_id === undefined || session_drink_id === undefined) {
        throw HTTPerror(400, 'Missing ID of the session or the drink within the session')
...
app.get('/api/tags', withDB(async (req, res, connection, next) => {
    ...
    res.status(200).json(updatedJson); // Return the results as JSON
}));
